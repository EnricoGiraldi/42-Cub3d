...
void	hud_draw(t_scene *scene)
{
	...
	tf = g_default2d;
	factor = (double)scene->rc_target->size.y / (double)MIN_WIN_H;
	tf.pos = dvec2_new((double)scene->rc_target->size.x / 2.0,
			(double)scene->rc_target->size.y);
	tf.sca = dvec2_scale(dvec2_new(1.5, 1.5), factor);
	if (scene->hud.arm)
		image_draw_into_tf(scene->rc_target, scene->hud.arm, tf);
	tf.pos.x = (double)scene->rc_target->size.x * 0.1;
	tf.pos.y = (double)scene->rc_target->size.y * 0.88;
	tf.sca = dvec2_scale(dvec2_new(4.0, 4.0), factor);
	if (scene->hud.hp)
		image_draw_into_tf(scene->rc_target, scene->hud.hp, tf);
	tf.pos.y = (double)scene->rc_target->size.y * 0.9;
	if (scene->hud.ammo)
		image_draw_into_tf(scene->rc_target, scene->hud.ammo, tf);
}
